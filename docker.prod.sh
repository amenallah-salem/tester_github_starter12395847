...
# Django and the database are perfectly healthy; the bare "/" also 404s
# (no view registered there). Use the dedicated, unauthenticated readiness
# endpoint instead.
...
if [ ! -f "$ENV_FILE" ]; then
  echo "Error: prod env file not found: $ENV_FILE" >&2
  exit 5
fi
...
# (e.g. POSTGRES_DB/POSTGRES_USER/POSTGRES_PASSWORD in the "environment:"
# block). The service-level "env_file: .env.prod" only injects variables
# into the *container's* runtime environment -- it does NOT make those
# values available for YAML substitution. Without --env-file here, those
...
if [ "$CMD" != "up" ]; then
  ...
  echo "Usage: ./docker.prod.sh [build|up] [--no-strict]" >&2
  exit 1
fi
...
while true; do
  ...
  # NOTE: in production DEBUG=False enables SECURE_SSL_REDIRECT, so any plain
  # HTTP request without a trusted "X-Forwarded-Proto: https" header gets a
  # 301 redirect from Django's SecurityMiddleware *before* it ever reaches
  # HealthCheckView. curl --fail treats 3xx as success, so without the header
  # this probe would report the backend "ready" without ever having checked
  # the database. Send the header the real reverse proxy would send so this
  # probe genuinely exercises the health view.
  ...
  if [ -n "$BACKEND_CID" ]; then
    if docker exec "$BACKEND_CID" sh -c "command -v curl >/dev/null 2>&1 && curl --fail --silent --max-time 5 -H 'X-Forwarded-Proto: https' http://127.0.0.1:8000${BACKEND_PATH}" >/dev/null 2>&1; then
...
# ------------------------------------------------------------------
# Only relevant when AI_PROVIDER=freellmapi (see .env.prod) and freellmapi
# runs on this same host — its own compose commonly publishes its port as
# 127.0.0.1:3001, which is loopback-only and unreachable from inside a
# container (host.docker.internal included), so the backend reaches it by
# joining its Docker network and using the container name instead
# (FREELLMAPI_BASE_URL=http://freellmapi:3001/v1). This is done here rather
# than declared in docker-compose.backend.prod.yml because Compose refuses
# to start a service that references a missing external network —
# declaring it there would break the whole prod stack for anyone not
# running freellmapi. Missing network or a failed connect (e.g. already
# joined) are both non-fatal.
